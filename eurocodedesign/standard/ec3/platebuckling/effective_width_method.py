...
@singledispatch
def bar_lambda_p(*args: Any) -> float:
    r"""Calculate modified slenderness for plate buckling
       ...
       Args:
           ...
           epsilon: epsilon factor :math:`\epsilon`, see
            ec3.crosssection.classification.epsilon
    ...
    """
    raise NotImplementedError(f"bar_lambda_p not supported for "
                              f"argument type {type(args)}")


@bar_lambda_p.register
def _bar_lambda_p_f_y(f_y: Pascal, sigma_crp: Pascal) -> float:
    r"""Calculate modified slenderness for plate buckling
...
@bar_lambda_p.register
def _bar_lambda_p_bar_b(bar_b: Meter,
                             t: Meter,
                             epsilon: float,
    ...
    Args:
        ...
        epsilon: Epsilon factor :math:`\epsilon`, see
            ec3.crosssection.classification.epsilon
        k_sigmap: plate buckling factor :math:`k_{\sigma,p}`, see k_sigmap
...
def rho_p(support: PlateSupport,
               bar_lambda_p: float,
               psi: float = 1.0) -> float:
...
def k_sigmap(psi: float, support: PlateSupport) -> float:
...
def sigma_crp(k_sigmap: float, sigma_E: Pascal) -> Pascal:
    ...
    Args:
        k_sigmap: plate buckling factor :math:`k_\sigma`, see k_sigmap
        sigma_E: maximum elastic stress :math:`\sigma_E`, see sigma_E
...
def effective_width(support: PlateSupport,
                         bar_b: Meter,
                         rho_p: float,
    ...
    Args:
        bar_b: Decisive width :math:`\bar{b}`
        rho_p: reduction factor :math:`\rho` for plate buckling, see rho_p
        psi: stress ratio :math:`\psi = \sigma_2 / \sigma_1`
    ...
    """
    if support != PlateSupport.TWO_SIDE:
        raise NotImplementedError("effective_width only implemented for"
...
def sigma_crc(t: Meter,
    ...
    """
    sigma_crc: Pascal = sigma_E(t, a)
    return sigma_crc


def bar_lambda_c(f_y: Pascal, sigma_crc: Pascal) -> float:
    ...
    Args:
        ...
        sigma_crc: critical stress :math:`\sigma_{cr,c}`
         for flexural flange buckling, see sigma_crc
...
def chi_c(stiffeners: PlateStiffeners,
    ...
    if stiffeners != PlateStiffeners.NONE:
        raise NotImplementedError("chi_c only implemented "
                                  "for unstiffened plates")

    chi_c: float = ec3.buckling.chi(bar_lambda_c,
                                         ec3.buckling.BucklingLine.a)
    return chi_c


def xi(sigma_crp: Pascal, sigma_crc: Pascal) -> float:
    r"""
...
def rho_c(rho_p: float, chi_c: float,
               xi: float) -> float:
    ...
    Args:
        ...
        chi_c: reduction factor :math:`\chi_c` for compression flange buckling,
            see chi_c
        xi: interaction factor :math:`\xi`, see xi
...
def is_shear_buckling_verification_required(h_w: Meter,
    ...
    epsilon_: float = epsilon(steel_grade.f_yk)

    if stiffeners == PlateStiffeners.NONE:
        return h_w / t_w > 72 / eta * epsilon_
    else:
        raise NotImplementedError("Not implemented for stiffened plates")


def k_tau(h_w: Meter, a: Meter, stiffeners: PlateStiffeners) -> float:
...
def tau_cr(k_tau: float, sigma_E: Pascal) -> Pascal:
    """Calculate :math:`\tau_{cr}`

    According to EN 1993-1-5:2019-10 §5.3(3)

    Args:
        k_tau: shear plate buckling factor :math:`k_\tau`, see k_tau
        sigma_E: yield stress :math:`sigma_E`, see sigma_E
...
def bar_lambda_w(f_yw: Pascal, tau_cr: Pascal) -> float:
...
def V_bw_Rd(chi_w: float, f_yw: Pascal, h_w: Meter,
    ...
    Args:
        chi_w: web reduction factor :math:`\chi_w`, see chi_w()
...
def eta_3(V_Ed: Pascal, V_b_Rd: Pascal) -> Eta:
