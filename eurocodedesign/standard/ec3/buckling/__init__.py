...
def _Phi(bar_lambda: float, buckling_line: BucklingLine) -> float:
...
def chi(bar_lambda: float, buckling_line: BucklingLine) -> float:
    # according to EC3-1-1 6.3.1.2 bar lambda relative slenderness
    chi: float
    if bar_lambda <= 0.2:
        chi = 1.0
    else:  # bar_lambda > 0.2
        Phi: float = _Phi(bar_lambda, buckling_line)
